...
while True:
    ...
    try:
        ...
        #check if number is between 1 - 50
        if 1 <= int(number) <= 10:
            numbers_1_10 += 1
        ...
        #Exits loop when number is more than 50 
        else:
            print(f"{'_'*50}\nNumber out of range, exiting...")
            break
    #Exits loop when input is invalid      
    except ValueError:
...
#Display the count and the collection of numbers for each range  
